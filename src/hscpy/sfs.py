...
def load_sfs(path2dir: Path, runs: int, timepoint: int = 1) -> Dict[str, Sfs]:
    ...
    sfs = dict()
    try:
        timepoint_path = dir_path_over_timepoint(measurement=Measurement.SFS, path2dir=path2dir, timepoint=timepoint)
...
def pandafy_sfs_dict(sfs_: Dict[str, Sfs]) -> pd.Series:
    ...
    for run_id, sfs in sfs_.items():
        sfs_df = pd.Series(sfs, dtype=int).value_counts().reset_index().rename(columns={"index": "# of j cells", "count": "# of muts in j cells"})
        sfs_df["id"] = run_id
